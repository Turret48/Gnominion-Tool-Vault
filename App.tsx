...
// --- Components defined internally to keep file count low while maintaining separation ---
...
const Sidebar = ({ 
  ...
  const getIcon = (cat: string) => {
    if (cat.includes(ToolCategory.AI)) return <Cpu size={18} />;
    if (cat.includes(ToolCategory.AUTOMATION)) return <Zap size={18} />;
    if (cat.includes(ToolCategory.DESIGN)) return <PenTool size={18} />;
    if (cat.includes(ToolCategory.EMAIL)) return <Mail size={18} />;
    if (cat.includes(ToolCategory.ANALYTICS)) return <BarChart2 size={18} />;
    return <Hash size={18} />;
  };
...
const AddToolModal = ({ 
  ...
  const handleChatSubmit = async (e: React.FormEvent) => {
    ...
    // Call Gemini
    const enriched = await enrichToolData(input);
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 ...
                 <div className="space-y-4">
                    ...
                    <datalist id="category-options">
                      {Object.values(ToolCategory).map(c => <option key={c} value={c} />)}
                    </datalist>
                 </div>
...
function App() {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (Array.isArray(imported)) {
           ...
           const currentUrls = new Set(tools.filter(t => t.url).map(t => t.url));
           
           const uniqueNewTools = imported.filter((t: Tool) => {
             // It's a duplicate if ID exists OR (URL exists and matches)
             const isIdDup = currentIds.has(t.id);
             const isUrlDup = t.url && currentUrls.has(t.url);
             return !isIdDup && !isUrlDup;
           });

           const addedCount = uniqueNewTools.length;
           const ignoredCount = imported.length - addedCount;

           if (addedCount > 0) {
             setTools(prev => [...uniqueNewTools, ...prev]);
           }
           
           alert(`${addedCount} tools added, ${ignoredCount} duplicate tools ignored.`);
        }
      } catch (err) {
  ...
  const categories = useMemo(() => {
    const cats = new Set(Object.values(ToolCategory));
    tools.forEach(t => cats.add(t.category as ToolCategory));
    ...
    <div className="flex min-h-screen bg-black text-gray-200 font-sans selection:bg-primary/30">
      ...
      {selectedToolId && selectedTool && (
        <ToolDetail 
          ...
          onRequestDelete={requestDeleteTool}
        />
      )}
    </div>
  );
